...
### If-Else-Endif
  - You can start condition statement with `if` and should close with `endif;`
  - When process passes if-statement, it runs statements after `:`.
...
### filter_input()
  ...
  -  Type is one of INPUT_GET, INPUT_POST, INPUT_COOKIE, INPUT_SERVER, or INPUT_ENV.
...
### addEventListener('submit', e => {});
  - When you click the submit button, it occurs an event. And with this event, you can do something in blank.

### createTextNode()
  - Create text node with specific value.

### querySelector: A more complex selector
  - For example, `var el = document.querySelector("div.user-panel.main input[name='login']");`. Here, the first `<input>` element with the name "login" (`<input name="login"/>`) located inside a `<div>` whose class is "user-panel main" (`<div class="user-panel main">`) in the document is returned.

### date('Y-m-d H:i:s', time());
  - `time()` returns current time as a milisecond.
  - `date()` transfers this milisecond to specific form.
...
### DOMParser.parseFromString()
  ...
  - mimeType can be text/html, text/xml, application/xml, application/xhtml+xml, or image/svg+xml.
